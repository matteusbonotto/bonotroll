...
**Fase 3 — Vitrine de vendas (no ar, mas ainda não divulgada, 2026-09-11)**
- `TASK-040` Landing page própria, copy de vendas/marketing em PT-BR, referência estética cinematográfica (dark, tipografia grande, scroll-reveal) — DONE, no ar em `/landing.html`. Decisão final de deploy: em vez de trocar a fonte do GitHub Pages pra uma branch separada (o que substituiria o app real na URL atual), a branch `landing-page` foi mergeada em `main` — `index.html` do app continua intocado na raiz, `landing.html`/`landing-assets/` passam a conviver no mesmo `main`, servidos pelo mesmo GitHub Pages, sem tirar o app do ar nem mexer em configuração do repositório.
- `TASK-041` (novo, **BLOCKED**): o formulário de lista de espera da landing só simula sucesso, não salva e-mail nenhum ainda (placeholder de propósito). **Decisão explícita do usuário (2026-09-11): não implementar isso agora nem divulgar a landing** — só depois de ter banco (esta task) e Stripe (`FEAT-005`) configurados.

**Fase 4 — Monetização**
- `FEAT-005` Preparação de Stripe/planos — agente `billing` já existe e pronto, mas a fase fica **BLOCKED** de propósito até o usuário decidir o modelo de plano/preço (o que é gratuito vs. pago). Nenhuma chave real é tocada antes disso. Esta fase agora também gateia o lançamento público da landing page (Fase 3), não só a monetização do app em si.
