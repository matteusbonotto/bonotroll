...
- **Cartão de crédito** (`cartao_credito` + `cartao_id` + categoria "Cartão de crédito"): uma saída marcada `cartao_credito = true` já está embutida no valor da "fatura" do mês (qualquer saída da categoria "Cartão de crédito", casada por nome, mesmo mês por `data_vencimento`/`data_cadastro`). `groupCartaoCredito` tira ela da lista de topo e pendura em `.filhosCartao` da fatura — quem soma (`computeSummary`, quebras/gráficos, comparativo de mês) nunca conta o mesmo dinheiro duas vezes; a fatura conta uma vez pelo valor dela. Sem fatura no mês, a despesa continua solta e contando normal (nunca desaparece). **Não** vale pra orçamento por categoria (`computeBudgetProgress` conta toda transação pela categoria dela, de propósito) nem pra "Contas a vencer" (dívida com data continua sendo dívida).
- **Multi-cartão** (2026-08-22, ver `.claude/discussions/001-cartao-credito-multi-cartao.md`): o agrupamento de `groupCartaoCredito` usa a chave `cartao_id` quando presente — fatura e compra com o **mesmo** `cartao_id` no mesmo mês se juntam, e duas faturas de cartões diferentes no mesmo mês **não** se misturam (era o bug de produção: agrupava pela primeira fatura do mês, ignorando de quem era). Sem `cartao_id` (dado legado), cai no fallback `responsavel_id + mês` — nunca mais "só mês". O cartão é uma entidade própria (`js/services/cartoes.js`, tabela `cartoes`), **pessoal** (owner-scoped, nome único por dono) e vinculada a um banco (`banco_id`); a leitura é compartilhada no grupo, a escrita é só do dono. No formulário de despesa, o antigo switch booleano virou um seletor de cartão: escolher um cartão liga `cartao_credito=true` e grava `cartao_id`; a opção "cartão não informado" (`__legado__`) mantém a marcação sem gravar cartão.
