...
## BUG-015 — Regressões críticas de CSS achadas pela auditoria de UX (não reportadas pelo usuário)

- **Data**: achado 2026-09-02, auditoria proativa pedida pelo usuário ("identificar gaps, erros, falhas, defeitos, e brechas que não estamos considerando").
- **Origem**: agente `ux-ui` (achado), agente `frontend` (correção).
- **Severidade**: CRITICAL — mesmo padrão de declaração-CSS-duplicada do BUG-003 (documentado no CLAUDE.md), reincidindo 3x mais depois daquela correção.
- **Status**: `FIXED`, verificado (90/90 unitários, 30/30 e2e).
- **Causa**: (C1) `.btn` (app.css) e `.cg-btn` (components.css, 2ª declaração duplicada) redefiniam `border-radius` pra `--radius-md`, vencendo por ordem sobre a pílula (`--radius-pill`) — o Design System de botão-pílula nunca renderizava de fato. (C2) `.cg-section-title` tinha valores soltos antigos em app.css vencendo a versão correta/documentada de components.css. (C3) `x-show` sem `.important` em cima da prévia de foto vazia em Compras (`novoItem.foto_url`/`itemForm.foto_url`) nunca escondia o wrapper — ícone de imagem quebrada visível sempre que não havia foto (esse é o mesmo padrão do BUG do `x-show`+Bootstrap-utility já documentado no CLAUDE.md, só que a "utility" aqui era o próprio Alpine sem modificador). (C4) card `.cg-card p-0` aninhado dentro do card externo de Grupo > "Entre vocês" (mesma família de erro do BUG-003 original, "card dentro de card"). (H1, achado durante a correção, não estava no diagnóstico original) `.form-control-color` (seletor de cor) virava 36×48 em vez de 36×36 porque `min-height` sempre estabelece um piso, e a regra de touch-target 48px vencia mesmo com `height:36px` inline mais específico.
- **Correção**: consolidada 1 declaração só de `.cg-btn` (mantendo pílula); removidas as propriedades duplicadas de `.cg-section-title`/`.form-control-color`(min-height:auto)/`.btn`(--bs-btn-border-radius) do app.css; `x-show` → `x-show.important` nos 2 pontos; `.cg-card p-0` removido de Grupo, substituído por `.cg-list-flat` direto + `border-top` como separador.
- **Prevenção**: sempre que uma auditoria encontra declaração duplicada, procurar TODAS as outras ocorrências do mesmo padrão no arquivo inteiro antes de corrigir só a reportada — 4 instâncias novas do mesmo erro já documentado (CLAUDE.md) apareceram de uma vez só, quase 2 semanas depois da correção original (BUG-003), sem terem sido pegas antes por não terem sintoma reportado pelo usuário (pílula "não renderizar" é sutil visualmente se ninguém comparar com o Design System).

## TASK-031 — Compras: reordenar botões (Editar→Excluir→Comprar) + rodapé na Grade + cabeçalho mobile compacto

- **Data**: 2026-09-02, pedido explícito do usuário (rodada de 7 melhorias pós-BUG-013/014).
- **Decisão não trivial registrada aqui**: o pedido novo ("editar | Excluir | comprar", Comprar por último) contradiz na superfície o espaçador físico do BUG-014 (lápis entre Comprar e Excluir, pra evitar erro de dedo) — reordenando, Excluir e Comprar voltam a ficar vizinhos. Decisão: seguir o pedido do usuário mesmo assim, porque a proteção REAL contra exclusão acidental não é mais a posição do botão, é o `notifyUndo()` (5s pra desfazer) já embutido em `removeItem()` desde o BUG-014 — o espaçador físico era só reforço de UX em cima disso, não a defesa principal. Se no futuro o usuário reportar exclusão acidental de novo em Compras, a causa NÃO será falta de espaçador (isso foi removido deliberadamente) — investigar primeiro se `notifyUndo` está mesmo disparando.
- **Status**: `DONE`. Ver `TASK-031` em `.claude/checklist/tasks.json`.
