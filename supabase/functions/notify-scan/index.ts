...
Deno.serve(async () => {
  ...
  // Avisa TODO o grupo (não só o responsavel_id) — pedido explícito: "se um
  // não ver, o outro vê e avisa". Sem grupo (despesa 100% pessoal), só o
  // responsável mesmo, não tem quem mais avisar.
  const { data: transacoes, error: txErro } = await supabase
    .from('transactions')
    .select('id, titulo, valor, responsavel_id, group_id, data_vencimento')
    .eq('tipo', 'saida')
  ...
  for (const t of transacoes ?? []) {
    ...
    const destinatarios = t.group_id ? await membrosDoGrupo(t.group_id) : [t.responsavel_id];
  ...
  let subsEncontradas = 0;
  if (vapidPublic && vapidPrivate) {
    for (const n of inseridas ?? []) {
      const { data: subs, error: subsErro } = await supabase.from('push_subscriptions').select('*').eq('profile_id', n.profile_id);
      if (subsErro) {
        pushErros.push({ profile_id: n.profile_id, motivo: `erro ao buscar inscrições: ${subsErro.message}` });
        continue;
      }
      subsEncontradas += subs?.length ?? 0;
  ...
  return new Response(
    JSON.stringify({
      ok: true,
      notificacoes: inseridas?.length ?? 0,
      subs_encontradas: subsEncontradas,
      push_enviados: pushEnviados,
      push_erros: pushErros,
    }),
