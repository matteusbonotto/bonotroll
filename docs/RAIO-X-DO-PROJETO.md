...
- Aparece pequena e discreta embaixo do saldo (`conversaoBRLFor`, "≈ R$ …") tanto no tile quanto no card de Saldo do detalhe, e a cotação em si (`cotacaoLabel`, "$ 1 ≈ R$ …" — símbolo da moeda, não o código) some discretamente perto da meta, no detalhe — nunca troca o valor principal, que continua sempre na moeda de verdade da caixinha.
- **`somaGeral`/`porResponsavel` somam tudo já convertido pra BRL** — caixinha em BRL entra direto, caixinha estrangeira entra pelo saldo × `taxasPorMoeda[moeda]` assim que a cotação carrega (e só fica de fora do total **temporariamente**, nunca com um valor inventado, enquanto a cotação daquela moeda específica ainda não chegou — texto "Buscando cotação…" avisa esse estado no card do total).

**Três bugs reais corrigidos no mesmo dia, todos encontrados pelo usuário em produção** (não em teste — o ambiente de teste tinha a mesma limitação de rede do primeiro bug, então não teria pego sozinho; os outros dois só apareceram depois de olhar o resultado real com atenção):
1. `api.frankfurter.app` foi descontinuado e passou a devolver **301** pra `api.frankfurter.dev/v1/...` — o `fetch` falhava (silenciosamente, por design do best-effort) em vez de seguir o redirect entre domínios de forma confiável. Corrigido apontando pro domínio novo direto.
2. **`somaGeral` somava o saldo BRUTO de qualquer caixinha, sem olhar a moeda** — uma caixinha de US$ 903 entrava na soma como se fosse R$ 903. `porResponsavel` já filtrava corretamente (só BRL); `somaSaldos` (a função usada por `somaGeral`) não. Primeira correção: filtrar moeda estrangeira pra fora da soma.
3. **Filtrar pra fora não era o comportamento certo** — o usuário apontou que uma caixinha em dólar devia **entrar no total, convertida**, não sumir dele. `somaSaldos` passou a aceitar o mapa de cotações e somar o valor já convertido em vez de excluir.

Todos verificados com uma chamada de rede de verdade (não só o modo demo): US$ 903 → "≈ R$ 4.699,30" no card da caixinha, "$ 1 ≈ R$ 5,20" de cotação, e total geral R$ 3.800,00 (BRL) + R$ 4.699,30 (USD convertido) = **R$ 8.499,30**, batendo exatamente.
