...
> Documentação técnica e funcional completa do projeto — banco de dados, arquitetura, cada tela, cada regra de negócio, design system e políticas de segurança. Escrita para ser a referência única de "como o Bõnotto funciona por dentro". Última atualização: 2026-08-19 (rodada de correções de notificações/push, service worker e auto-refresh; depois, no mesmo dia, uma segunda rodada bem maior: módulo novo de Caixinhas, recorrência com cadência/parcelas, agrupamento em accordion nas Transações, gráfico por membro, ícone de empresa por URL, importação de CSV para Recursos, correção da duplicidade de categorias e dois bugs reais de sincronização de estado).
...
#### `categories`
Pessoal (`owner_id`) ou de grupo (`group_id`), nunca as duas coisas ao mesmo tempo por linha — mas visível pra quem é dono OU membro do grupo. Índice único em `(owner_id, coalesce(group_id, '00000000...'), lower(btrim(nome)))` evita duplicata do mesmo nome — **normalizado (case/espaço-insensível) desde 2026-08-19**: a versão anterior comparava o texto literal (`"Casa"` ≠ `"casa"` ≠ `"Casa "`), o que deixava passar duplicatas que o próprio app já tratava como iguais ao comparar (`ensureDefaultCategories`, `guessCategoryByTitle`). Migração inclui limpeza automática dos duplicados por maiúscula/espaço que já existiam antes da correção (mesmo padrão de reapontar-e-apagar já usado pro caso de duplicata exata).
...
Dinheiro guardado em bancos/corretoras (2026-08-19) — ver §20 pro módulo completo. `caixinhas` guarda só a config (`banco_nome`, `moeda`, `meta`, `icone`); guardado/retirado/saldo **nunca são colunas**, são sempre somados a partir de `caixinha_movimentacoes` (mesmo princípio de "status não é coluna" de `transactions` acima). `caixinha_movimentacoes.tipo`: `'guardado' | 'retirado'`.
...
- Filtros: tipo, categoria, responsável, status, fixa/variável, busca por texto, período (com atalhos: hoje±N dias, mês atual). Painel de filtros é **recolhível só no mobile** (`filtrosAbertos`) — sempre aberto em telas ≥992px, porque a versão anterior sempre-aberta consumia a tela inteira no celular.
...
- **4 modos de visualização** (`viewMode`, persistido por navegador): lista (tabela desktop / cards empilhados mobile, com edição inline), grade e grade compacta (navegação — tocar abre o formulário completo em vez de editar inline), e **Agrupado** (2026-08-19 — ver §8.4).
...
### 8.5 Agrupado: accordion por período/responsável/movimentação/categoria (2026-08-19)

"Feature mega importante" pedida explicitamente: um 4º modo de visualização (`viewMode = 'agrupado'`) que organiza as transações filtradas num accordion, com resumo por grupo e abrir/fechar independente.

- **Por período** (`gruposPorPeriodo`, 2 níveis): ano → mês (`aaaa-mm`, rótulo tipo "ago/26"), usando `data_vencimento` (ou `data_cadastro` se não tiver vencimento; sem nenhuma das duas, cai num grupo "Sem data" à parte em vez de sumir). O mês/ano **atual** vem aberto por padrão (`abertoPorPadrao`/`isAtual`), com uma tag "Agora" — o resto começa fechado.
- **Por responsável / Por movimentação / Por categoria** (`gruposSimples`, 1 nível só): mesmo componente visual, um grupo por valor distinto do campo escolhido, ordenado do maior pro menor.
- **Resumo por grupo** (`resumoGrupo`): soma de entradas/saídas + contagem por status (pago/vencido/a vencer/pendente), reaproveitando `STATUS_META` (mesmas cores/ícones do badge de status de sempre) — é o "Ent. R$, Saí. R$, N pagos, N vencidos…" pedido no exemplo original.
- **Abrir/fechar**: `overridesAbertura` (objeto plano, não Map — mesmo padrão reativo de `payersByTx`) guarda só as exceções ao "aberto por padrão" de cada grupo; `estaAberto(chave, padrao)`/`toggleGrupo(chave, padrao)` leem/gravam nele. Trocar de dimensão (`setAgrupamento`) zera os overrides.
...
Como `schema.sql` só roda quando a pessoa decide rodar manualmente, é comum uma conta real estar "atrasada" em relação ao código (uma coluna nova já existe no app, mas ainda não no banco daquela pessoa). Em vez de travar a ação inteira com erro de coluna inexistente, `comFallbackDeColuna()` detecta o erro `PGRST204`/"column does not exist" do PostgREST, remove **só aquela coluna** do objeto sendo salvo, e tenta de novo — até 3 vezes. Usado em `createTransaction`/`updateTransaction` e nos inserts/updates de itens de lista de compras.
...
- **Grade** (`!activeId`): um tile por caixinha (reaproveita `.cg-room-tile`/`.cg-room-grid` de Recursos), com saldo e — quando a caixinha tem `meta` definida — uma barrinha de progresso (`computeProgresso`, 0–100%, `null` sem meta). Lápis de editar no canto, mesmo padrão de `abrirEditarRoom`.
- **Detalhe** (`activeId`): 4 cards de métrica (Guardado, Retirado, Saldo, Meta com % e barra), botões "Guardar"/"Retirar" (abrem o mesmo modal de movimentação com `tipo` pré-selecionado), e o histórico completo (mais recente primeiro), cada linha removível.
...
- **Soma geral** (`somaGeral`): saldo de TODAS as caixinhas visíveis somado (`somaSaldos`) — cobre "quanto tem guardado no total, entre os dois".
- **Por responsável** (`porResponsavel`): uma linha por membro (avatar + nome + soma só das caixinhas cujo `owner_id` é aquele membro) — cobre "quanto o responsável tem individual e somado com os membros do grupo" (o "somado" é justamente `somaGeral`, ao lado).

### 20.4 Schema

`caixinhas`: `owner_id`, `group_id` (opcional), `banco_nome`, `moeda` (default `'BRL'`), `meta` (opcional), `icone`. `caixinha_movimentacoes`: `caixinha_id`, `tipo` (`'guardado' | 'retirado'`), `valor`, `data`, `observacoes`. RLS espelha exatamente o padrão de `resource_rooms`/`resource_items` (owner ou membro do grupo, `is_group_member`).
...
O dia 2026-08-19 concentrou duas rodadas de trabalho bem distintas. A primeira consolidou notificações/push como infraestrutura real e confirmada (ver §11) e resolveu uma leva de bugs de UI menores. A segunda, bem maior, nasceu de um único pedido do usuário com mais de dez itens e entregou: um módulo novo inteiro (Caixinhas, §20); recorrência com cadência configurável (semanal/mensal/anual/personalizada) e contagem de parcela (§8.3); um 4º modo de visualização em Transações — accordion agrupado por período/responsável/movimentação/categoria com resumo por grupo (§8.5); um gráfico novo no dashboard ("por membro do grupo", §7.1); logo de empresa/serviço por URL além de upload; importação de CSV estendida pra Transações (campos de responsável/status/observações/parcela que antes não tinham pra onde ir) e um alvo totalmente novo pra Recursos, com um arquivo de produtos essenciais pronto pra importar; e a correção definitiva da duplicidade de categorias (índice único do banco passou a normalizar case/espaço, igual ao app já fazia). No meio do caminho, duas investigações renderam bugs reais e não-óbvios corrigidos: o `<select>` de mapeamento da importação de CSV nunca refletia visualmente o auto-match (afetava os três alvos de import, não só o novo), e a tela de Recursos não se atualizava quando um item era criado por fora dela (a importação de CSV, por exemplo) — ambos confirmados por teste direto em navegador, não só por leitura de código.
