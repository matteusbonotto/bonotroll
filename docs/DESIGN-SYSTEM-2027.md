...
- ✅ Separação navegação/ação/destrutiva: auditada e **já estava correta** — toda ação de excluir no app usa `btn-outline-danger` (nunca preenchido), nunca com o mesmo peso visual de uma ação primária. O vermelho preenchido em "Nova despesa"/"Despesa" é uma dimensão semântica diferente (cor por sentido do lançamento — saída é vermelho, entrada é verde — um padrão comum em apps financeiros), não confundível com destrutivo na prática; nenhuma mudança necessária aqui.
